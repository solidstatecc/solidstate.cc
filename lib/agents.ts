...
export const agents: AgentRuntime[] = [
  ...
  },
  {
    ...
    description:
      "xAI's Grok. The consumer app (web, iOS, Android) runs Skills you build or upload at grok.com/skills. Grok Build, the coding CLI, is fully Claude Code-compatible with zero config: it reads Claude Code skills, plugins, marketplaces, MCPs, and CLAUDE.md alongside its own .grok/ directories.",
    skillInstall:
      "App (web/iOS/Android): download a skill's .skill from Solid State, then grok.com/skills -> New Skill -> Upload skill file. Skills is a paid feature (SuperGrok / X Premium+). Build (CLI): zero config if you use Claude Code -- Grok reads ~/.claude/skills/ and Claude marketplaces (~/.claude/plugins/known_marketplaces.json); or drop a folder into ~/.grok/skills/. Verify CLI discovery with `grok inspect`.",
    skillsDir: "Grok app: grok.com/skills - CLI: ~/.grok/skills/ - ~/.claude/skills/",
    ...
    ssMarketplace: {
      ...
      how: "Grok Build TUI: /plugins -> Marketplace -> add source",
      note: "Installs the Solid State plugin -- 5 voice skills (voice-check, anti-buzzword, contrast-pair, reframe, compress). Verified live in Grok Build 2026-06-14. The full indexed catalog runs via Claude-compat, no install needed.",
    },
    openSource: false,
  },
  {
