...
then name skills in the request's `container.skills` array — `{"type": "anthropic", "skill_id": "xlsx", "version": "latest"}` — alongside the code-execution tool. Custom skills upload once via the [Skills API](https://docs.claude.com/en/api/skills-guide) and load the same way. Full syntax on the [install page](/index#claude-api).
